...
Add `force_refresh: true` (25 credits) to get the most current result instead of a recent stored one. `force_refresh` applies to deep verification only — it is ignored on quick verifications.
