...
### Changed
- `POST /v1/checker` (the free, no-signup business checker) now enforces a small shared capacity ceiling and a per-IP daily limit, alongside the existing per-IP per-minute rate limit. When free checking is momentarily at capacity, the endpoint returns HTTP `429` with a new error code — `FREE_TIER_CAPACITY` — and a `Retry-After` header of a few seconds; this is a transient free-tier state, not an API outage and not a caller error. Exceeding the per-minute or per-day IP limit continues to return `429 RATE_LIMIT_EXCEEDED`, which now also carries a `Retry-After` header. Requests made with an API key are unaffected — the ceiling applies to the free checker only. SDKs need no update.

## 2026-07-23

### Changed
- A confirmed no-match on `POST /v1/verify` is now billed as a completed verification instead of being refunded. The lookup runs against the current official record and establishes that the business is not registered in that jurisdiction — that finding is the result you asked for, so it consumes credits like any other verification (quick: 1 credit, deep: 15). The response is unchanged: still `404 ENTITY_NOT_FOUND`. Genuine failures are unaffected and continue to refund automatically — `503 DATA_SOURCE_UNAVAILABLE`, `504 DATA_SOURCE_TIMEOUT`, and asynchronous jobs that fail after all retry attempts. No request or response shape changes; SDKs need no update.
